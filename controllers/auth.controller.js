...
const { User, Admin } = require("../models"); // Admin modelini ham chaqiramizz
...
exports.login = async (req, res) => {
  try {
    const { telegramId, firstName, username, photo_url, start_param } = req.body;
    ...
    if (!user) {
      ...
      // --- REFERRAL BONUS (yangi tizim) ---
      if (start_param) {
        try {
          const referralController = require("./referral.controller");
          await referralController.processReferral(user, String(start_param));
          await user.reload();
        } catch (refErr) {
          console.error("Referral xatosi (davom etadi):", refErr.message);
        }
      }
    } else {
